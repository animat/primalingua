...
while ($question = mysql_fetch_array($query)) {
	$q_id = $question['id'];
	$query2 = mysql_query("SELECT * FROM answers WHERE q_id = '$q_id' and student_id = '$student'");
	while ($answer = mysql_fetch_array($query2)) {
		$resp = $resp.'"q_'.$q_id.'":"'.$answer['content'].'",';
	}
	$query3 = mysql_query("SELECT * FROM feedback WHERE q_id = '$q_id' and student_id = '$student'");
	while ($answer2 = mysql_fetch_array($query3)) {
		$resp = $resp.'"f_'.$q_id.'":"'.$answer2['content'].'",'.'"f_'.$q_id.'_status":"'.$answer2['type'].'",';
