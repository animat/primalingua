...
var RoCanvas= function () {	
	...
	this.RO = function(id, vars) {		
		...
		toolBarDOM.className = "drawing_tools"		
		...
		if(self.toolbar.clearButton || self.toolbar.saveButton)
		{
			...
			if(self.toolbar.saveButton)
			{
				...
				toolBarHTML+='<input type="button" class="drawing_save_btn" id="RoCanvasSave_'+ this.id +'" value="'+self.toolbar.saveButton.text+'"'+saveButtonCallback+' />';
			}			
			toolBarHTML+="</p>";
		}
		...
		self.canvas.addEventListener('mousemove', function(e)
		{
		    ...
		    {		    	
				...
				switch(self.drawTool)
				{
					case 'rectangle':		
					case 'filledrectangle':		
						w = e.pageX - $(this).offset().left - self.startX;
						h = e.pageY - $(this).offset().top - self.startY;
			        ...
			        case 'circle':
			        case 'filledcircle':
			            w = Math.abs(e.pageX - $(this).offset().left - self.startX);
			            h = Math.abs(e.pageY - $(this).offset().top - self.startY);
			        ...
			        break;
					default:
						self.addClick(e.pageX - $("#" + id).offset().left, e.pageY - $("#" + id).offset().top, true);
					break;
			}
